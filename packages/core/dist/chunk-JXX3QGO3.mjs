...
async function loadMcpTools(ids) {
  ...
  for (const [serverId, _toolIds] of serverGroups) {
    try {
      ...
      if (!mcpClient) {
        const { getServerDataAdapter: getServerDataAdapter2 } = await import("@vibex/data");
        const createAISdkMcpClient = null;
        if (!createAISdkMcpClient) {
          console.warn(`[Tools] MCP support not yet implemented`);
          continue;
        }
        const adapter = getServerDataAdapter2();
...
`;
  ...
  static async start(goal, options = {}) {
    ...
    const { startSpace: startSpace2 } = await import("./space-4333FMYF.mjs");
