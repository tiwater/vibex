...
async function buildToolMap(toolIds, context) {
  ...
  if (customToolIds.length > 0) {
    try {
      ...
      console.warn(`[Tools] Failed to load custom tools from @vibex/tools:`, error);
    }
  }
...
async function loadMcpTools(ids) {
  ...
  for (const [serverId, _toolIds] of serverGroups) {
    try {
      ...
      if (!mcpClient) {
        console.warn(`[Tools] MCP support not yet implemented for server: ${serverId}`);
        continue;
      }
    } catch (error) {
...
Use "condition" for decision points.`,
   ...
   */
  async handleParallelExecution(agentIds, messages, systemMessage, _spaceId, metadata, _restOptions) {
   ...
   */
  _extractPromptFromMessages(messages) {
  ...
  handleMessagePersistence(streamResult, messages, spaceId, metadata) {
    (async () => {
      try {
        ...
        const _updatedMessages = [...messages, assistantMessage];
      } catch (error) {
...
`;
  ...
  static async start(goal, options = {}) {
    ...
    const { startSpace: startSpace2 } = await import("./space-ZA25NT65.mjs");
